...
exports.terapiData = async (req, res, next) => {
  ...
  if (Object.keys(req.body).length != 0) {
    try {
      const terapi = await Vibio_terapi.create({ uuid: uniqueID, uuid_user: req.params.uuid_user, json_data: JSON.stringify(req.body.json_data), terapi: req.body.tipe_terapi });
      if (terapi) {
        res.json({
          status: "success",
          code: 200,
